...
dbook_title="AFA" # Idealmente seria $1
...
#sos_gen_diagrama(){
  ...
  #echo "<chapter id=\"$chapter_name\"><title>Diagrama de la Solucion</title><section id=\"$chapter_name.grafico\"><title>$chapter_name</title><para><informalfigure><graphic fileref=\"$diag_img\" scalefit=\"1\" width=\"100%\" contentdepth=\"100%\"/></informalfigure></para></section></chapter>" >> $dbook_data_dir/$chapter_name.xml
#}
...
if [ -e $publican_test_dir/$dbook_title ] ; then
  echo "$dbook_title ya existe. Renombrar este documento o borrar $dbook_title"
  exit 1
else
  mkdir $publican_test_dir/$dbook_title &>/dev/null
  mkdir $publican_test_dir/$dbook_title/en-US &>/dev/null
  #touch $publican_test_dir/publican.cfg &>/dev/null
  echo "xml_lang: $xml_lang" >> $publican_test_dir/$dbook_title/publican.cfg
  echo "type: $dbook_type" >> $publican_test_dir/$dbook_title/publican.cfg
  echo "brand: $brand" >> $publican_test_dir/$dbook_title/publican.cfg 
fi
